...
export default async function handler(
  ...
  if (req.method === "GET") {
    ...
    try {
      const { full_name, username, email, password, gender, image, role } =
        req.body;
  ...
  OUTPUT inserted.*
  VALUES (${full_name}, ${username}, ${email}, ${hashedPassword}, ${gender},${role} ,${
        image ?? ""
      });
`;
      res.status(201).json({
    ...
    try {
      const { user_id, username, full_name, gender, password, role, image } =
        req.body;

      if (!user_id || !username || !full_name || !gender) {
      ...
      const isSamePassword = await bcrypt.compare(password, theUser.password);

      const hashedPassword = isSamePassword
        ? password
