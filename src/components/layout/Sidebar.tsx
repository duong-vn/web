...
const Sidebar = () => {
  ...
  return (
    <div className="fixed left-0 top-0 h-screen bg-white border-r border-gray-100 transition-all duration-300 z-40">
      <div className={`h-full flex flex-col transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}>
        {/* Toggle Button */}
        <button
          onClick={() => setIsCollapsed(!isCollapsed)}
          className="absolute -right-3 top-6 bg-white border border-gray-100 rounded-full p-1.5 shadow-sm hover:bg-gray-50 transition-all duration-200 hover:scale-110 z-50"
        >
          {isCollapsed ? (
        ...
        {/* Logo/Brand */}
        <div className="p-6">
          <h1 className={`text-xl font-semibold text-gray-900 transition-all duration-300 ${isCollapsed ? 'opacity-0' : 'opacity-100'}`}>
            SocialHub
          </h1>
        </div>
